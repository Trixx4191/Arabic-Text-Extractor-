...
def main(args):
    ...
    for file in tqdm(pdf_files, desc="Processing PDFs"):
        try:
            ...
            if not args.no_translate and arabic_lines:
            ...
            logger.info(f"Saved DOCX (RTL): {docx_path}")
            
        except Exception as e:
            logger.error(f"Processing {file} failed: {e}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Advanced Arabic PDF Text Extractor')
    ...
    parser.add_argument('--force-ocr', action='store_true', help='Force OCR even if native text available')
